  ...
  return snapshot.docs
    .map((doc) => {
      ...
      return {
        ...
        houseNumber: typeof data.houseNumber === "string" ? data.houseNumber : undefined,
        postalCode: typeof data.postalCode === "string" ? data.postalCode : undefined,
        land: typeof data.land === "string" ? data.land : undefined,
...
export const createBooking = async (booking: NewBooking): Promise<Booking> => {
  const docRef = await getDb().collection(BOOKINGS_COLLECTION).add({
    ...
    houseNumber: booking.houseNumber ?? null,
    postalCode: booking.postalCode ?? null,
    land: booking.land ?? null,
